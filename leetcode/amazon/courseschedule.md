...
class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        self.done = [False]*numCourses
        for currCourse in range(numCourses):
            if self.iscycle(currCourse, adjlist, vis):
                return False
        return True
    
    
    def iscycle(self, currCourse, prerequisite, vis):
        if self.done[currCourse]:
            return False
        ...
        ret = False
        for course in prerequisite[currCourse]:
            ret = self.iscycle(course, prerequisite, vis)
            if ret:
                break
                
        vis[currCourse] = False
        self.done[currCourse] = True
        return ret
